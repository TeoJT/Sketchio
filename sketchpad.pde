...
public class Sketchpad extends Screen {
  ...
  private void beginRendering() {
    ...
    //if (upscalePixels != 1.) {
    //}
    swapchain = new SwapchainCanvas[SWAPCHAIN_SIZE];
  ...
  private void runCanvas() {
    ...
    if (rendering && !converting && successful.get() && !compiling.get()) {
      ...
      // We initiate swapchain canvases as we render because due to that god ass annoying bug in processing
      // which causes an unavoidable stalls when we create large images, we don't want to freeze the application
      // for a long time and instead have more, but shorter freezes.
      if (swapchain[swapchainIndex] == null) {
        swapchain[swapchainIndex] = new SwapchainCanvas();
        
        // init shader canvas 
        swapchain[swapchainIndex].shaderCanvas = createGraphics(canvas.width, canvas.height, P2D);
        ((PGraphicsOpenGL)swapchain[swapchainIndex].shaderCanvas).textureSampling(2);   // Disable texture smoothing
        swapchain[swapchainIndex].shaderCanvas.hint(DISABLE_ASYNC_SAVEFRAME);
        
        // init scale canvas
        swapchain[swapchainIndex].scaleCanvas = createGraphics(int(canvas.width*upscalePixels), int(canvas.height*upscalePixels));
        //((PGraphicsOpenGL)swapchain[swapchainIndex].scaleCanvas).textureSampling(2);   // Disable texture smoothing
        swapchain[swapchainIndex].scaleCanvas.hint(DISABLE_ASYNC_SAVEFRAME);
      }
      
      // Before we go rendering anything, we need to make sure our thread is free and not writing anything
      // If busy, unfortunately we'll have no choice but to wait until the thread is free.
      int timewaited = 0;
      // Note: once thread is ready, we claim the thread by setting false automatically.
      while (!swapchain[swapchainIndex].ready.compareAndSet(true, false)) {
        
        // Try to search for a free thread to use instead
        boolean success = false;
        for (int i = 0; i < SWAPCHAIN_SIZE; i++) {
          if (swapchain[i] != null && swapchain[swapchainIndex].ready.compareAndSet(true, false)) {
      ...
      // Just debug (and error catching) info.
      if (timewaited > 5000) {
        console.warn("Something happened. Thread stall over 5 secs, could not render animation.");
        rendering = false;
        return;
      }
      else if (timewaited > 0) {
        console.log("Busy thread, had to stall for "+timewaited+"ms.");
      }
      
      // Begin drawing to the shader canvas.
      // Of course, TODO: we don't have shaders yet.
      swapchain[swapchainIndex].shaderCanvas.beginDraw();
      swapchain[swapchainIndex].shaderCanvas.clear();
      swapchain[swapchainIndex].shaderCanvas.image(canvas, 0, 0, swapchain[swapchainIndex].shaderCanvas.width, swapchain[swapchainIndex].shaderCanvas.height);
      swapchain[swapchainIndex].shaderCanvas.endDraw();
      
      // Do scaling (yay)
      // But if we don't have scaling enabled skip this step, will save performance and time.
      if (upscalePixels != 1) {
        swapchain[swapchainIndex].scaleCanvas.beginDraw();
        swapchain[swapchainIndex].scaleCanvas.clear();
        swapchain[swapchainIndex].scaleCanvas.image(swapchain[swapchainIndex].shaderCanvas, 0, 0, swapchain[swapchainIndex].scaleCanvas.width, swapchain[swapchainIndex].scaleCanvas.height);
        swapchain[swapchainIndex].scaleCanvas.endDraw();
        
        image(swapchain[swapchainIndex].scaleCanvas, 0, 50, 300, 300);
        
        final int index = swapchainIndex;
        final String myframe = frame;
        swapchain[index].savingThread = new Thread(new Runnable() {
              public void run() {
                swapchain[index].scaleCanvas.save(myframe);
                // Once done release the chain slot
                swapchain[index].ready.set(true);
              }
        });
        swapchain[index].savingThread.start();
      }
      else {
        
        image(swapchain[swapchainIndex].shaderCanvas, 0, 50, 300, 300);
        ...
        swapchain[index].savingThread = new Thread(new Runnable() {
              public void run() {
                swapchain[index].shaderCanvas.save(myframe);
                // Once done release the chain slot
                swapchain[index].ready.set(true);
              }
        });
      ...
      // After all this, move on to the next swapchain slot
      swapchainIndex++;
      if (swapchainIndex >= SWAPCHAIN_SIZE) swapchainIndex = 0;
    }
  ...
  private void beginConversion() {
    ...
    if (upscalePixels != 1.) {
      if (swapchain == null || swapchain[0].scaleCanvas == null) {
        console.bugWarn("beginConversion: swapchain and scaleCanvas shouldn't be null here!");
        return;
      }
      
      // Just use first scalecanvas in swapchain
      // ugly code but ive commented it here so you know what it does now.
      wi = swapchain[0].scaleCanvas.width;
      hi = swapchain[0].scaleCanvas.height;
    }
