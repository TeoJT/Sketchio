...
# sketchio_packer.py
# Used for when exporting a release of sketchio for uploading on Github,
...
# Check if the folder we're in is "sketchio"
...
dir = dir[len(dir)-1]
# check if it's "sketchio"
if (dir != "Sketchio"):
    print(color.RED+"Error: Sketchio directory not found."+color.NONE)
    exit(1)
...
print(color.NONE)
print(color.GOLD+color.UNDERLINE+"Correction: it should be called SketchioPacker but I'm too lazy to change it.")
print(color.NONE)
...
for arg in sys.argv:
    if (arg == "-s"):
        ...
        print(color.GOLD+"Usage: python3 sketchio_packer.py [-s][-h][-o]"+color.NONE)
...
new_name_windows = "sketchio_windows_"+version
if (os.path.isdir("application.windows64")):
    # rename to sketchio_windows_(version)
...
# If a folder with files called sketchio_windows exists from a previous packaging, delete it.
if (os.path.isdir("sketchio_windows")):
    shutil.rmtree("sketchio_windows")

windows_executable_location = "null"
windows_stability = 0
windows_download_size = 1  # Needs to be at least 1kb
if (os.path.isdir("sketchio_windows_"+version)):
    actionsComplete = True
    windows_stability = 5
    windows_executable_location = "sketchio_windows_"+version+"/Sketchio.exe"
    # move all .dll files in sketchio_windows_(version)/lib (apart from dsj.dll) to windows64
    ...
    if (not skipPackaging):
        ...
        if (zip):
            ...
            zip_source = "sketchio_windows"+"/"+new_name_windows
            if (not os.path.isdir("sketchio_windows")): 
                os.mkdir("sketchio_windows")
            os.rename(new_name_windows, zip_source)

            shutil.make_archive(new_name_windows, 'zip', "sketchio_windows")
...
new_name_linux = "sketchio_linux_"+version

# Legacy Processing 3 name
if (os.path.isdir("application.linux64")):
    # rename to sketchio_linux_(version)
...
""".format(
    info_version="0.1",
    sketchio_name=sketchio_NAME,
    update_name=sketchio_NAME+" "+version_original,
